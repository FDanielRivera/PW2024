...
namespace ProyectoMuebleria.Controllers
{
    ...
    public class ProductosController : Controller
    {
        ...
        {
            var categorias = await _miContexto.Producto.ToListAsync();
        ...
        {
            bool existe = await _miContexto.Producto.Where(c => c.IdProducto == idProducto).AnyAsync();
            if (existe)
            {
                Producto producto = await _miContexto.Producto.Where(c => c.IdProducto == idProducto).FirstOrDefaultAsync();
                return Ok(producto);
            }
            else
            {
            ...
            [FromForm] decimal precioAgregar)
        {
            ...
            try
            {
                _miContexto.Producto.Add(nuevoProducto);
